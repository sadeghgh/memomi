...
class HomeListTile extends StatefulWidget {
  ...
  List friends;
  HomeListTile(this.data, this.friends, {Key? key}) : super(key: key);
...
class _HomeListTileState extends State<HomeListTile> {
  ...
  Widget build(BuildContext context) {
    return widget.data['showMode'][0] == 'showForMyFriends'
        ? myCard()
        : Container(
            height: 0,
          );
  ...
  myCard() {
    int count = widget.friends.length;
    for (int i = 0; i <= count; i++) {
      if (widget.friends[i] == widget.data['ownerId']) {
        GestureDetector(
            onTap: () {
